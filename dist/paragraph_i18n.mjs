...
class n {
  ...
  constructor({
    ...
    this.api = a, this.readOnly = s, this.activeLanguage = r, this.autoTranslate = o, this._CSS = {
  ...
  drawView() {
    const t = document.createElement("DIV");
    return t.classList.add(this._CSS.wrapper, this._CSS.block), t.contentEditable = "false", t.dataset.placeholderActive = this.api.i18n.t(this._placeholder), this._data.translations[this.activeLanguage] && this._data.translations[this.activeLanguage].text && (t.innerHTML = this._data.translations[this.activeLanguage].text), this.readOnly || (t.contentEditable = "true", t.addEventListener("keyup", this.onKeyUp)), t;
  }
  /**
   * Return Tool's view
   *
  ...
  merge(t) {
    if (!this._element)
      return;
    this._data.translations[this.activeLanguage].text += t.translations[this.activeLanguage].text;
    const e = h(t.translations[this.activeLanguage].text);
   ...
   */
  validate(t) {
    return !(t.translations[this.activeLanguage].text.trim() === "" && !this._preserveBlank);
  }
  async translate(t) {
    if (!this.autoTranslate)
      return !1;
    const e = this._data.translations[this.activeLanguage].text, a = await this.autoTranslate(
      e,
      this.activeLanguage,
      t
    );
    return this._data.translations[t] || (this._data.translations[t] = { text: "" }), this._data.translations[t].text = a, !0;
  }
  /**
  ...
  onPaste(t) {
    ...
    this._data.translations[this.activeLanguage] = e, window.requestAnimationFrame(() => {
      this._element && (this._element.innerHTML = this._data.translations[this.activeLanguage].text || "");
    });
  }
  /**
